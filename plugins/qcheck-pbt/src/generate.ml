...
let print_ir ~module_name namespace s =
  let open Ortac_core in
  let context = Context.init module_name namespace in
  Ir_of_gospel.signature ~context s
...
let generate path fmt =
  ...
  let ir = print_ir ~module_name (List.hd env) sigs in
  let _structure = ir.structure in
  let _types = ir.types in
